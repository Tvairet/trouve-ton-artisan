...
// phpMyAdmin export → on récupère la table artisans
function extractRows(rawJson) {
  if (Array.isArray(rawJson)) {
    const table = rawJson.find(
      (x) => x?.type === "table" && x?.name === "artisans"
    );
...
//  slugs utilisés par ton front
function mapCategory(Id) {
  const x = (Id);
  if (x=2) return "batiment";
  if (x=4) return "services";
  if (x=3) return "fabrication";
  if (x=1) return "alimentation";
  return slugify(x);
}

// Normalisation d’une ligne de la vue → shape pour les cards/pages
function normalize(row) {
  const note = Number.parseFloat(row.grade ?? "0") || 0; // ex: "4.5"
